...
def generate_fraud_dataset(n_samples=100, fraud_rate=0.05):
    ...
    # Initialize account balances (random for customers, 0 for merchants)
    account_balances = {}
    ...
    for i in range(n_samples - fraud_target - 5):  # Reserve space for pattern frauds
        ...
        if tx_type == 'PAYMENT':
            ...
            # CASH_IN: External -> Customer
            nameOrig = 'EXTERNAL'
            nameDest = random.choice(customer_accounts)
        elif tx_type == 'CASH_OUT':
            # CASH_OUT: Customer -> External
            nameOrig = random.choice(customer_accounts)
            nameDest = 'EXTERNAL'
        else:  # DEBIT
            # DEBIT: Customer -> Merchant
            nameOrig = random.choice(customer_accounts)
            nameDest = random.choice(merchant_accounts)
        
        # Get current balances
        oldbalanceOrg = account_balances.get(nameOrig, 0.0) if nameOrig != 'EXTERNAL' else 0.0
        oldbalanceDest = account_balances.get(nameDest, 0.0) if nameDest != 'EXTERNAL' else 0.0
        
        # Generate amount
        base_amount = exponential_random(50)
        if nameOrig in account_balances and nameOrig != 'EXTERNAL':
        ...
        if nameDest in account_balances and nameDest != 'EXTERNAL':
